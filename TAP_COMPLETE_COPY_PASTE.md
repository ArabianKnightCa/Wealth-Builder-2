# TAP (Text Adaptation Processor) - Complete Code Package

## Overview
TAP v2.3.1 - Formula-driven text adaptation system for financial education content.
- NO age buckets (continuous age 6-99)
- NO experience buckets (discrete rungs EL 1-5 for POC)
- Formula-based transformations (LC, CD, IA scalars)
- Grammar-safe rewrite pipeline
...
Due to length constraints, the following files are also part of TAP:

### tap_v2_3_templates.py (176 lines)
- ContentBlock class with threshold-based reveal
- ContentTemplate class for progressive content
- create_sample_template() function

### tap_v2_3_1_language.py (590+ lines)
- SafeRewritePipeline class (8-step pipeline)
- Invariant protection (numbers, entities, phrases)
- Sentence normalization by LC level
- CD-based clarification
- IA-based framing
- Grammar-safe reflow
- Validation system

### tap_v2_3_ae_integration.py (166 lines)
- AEStatePacket dataclass
- AEModifier class
- Friction/momentum/exposure handling

### tap_v2_3_language.py (516 lines - LEGACY)
- Original language shaper (pre-pipeline)
- Still available if USE_SAFE_REWRITE_PIPELINE=false
...
result = engine.transform_content(
    baseline_text="When making financial decisions, I prefer to research extensively before deciding",
    user_age=35,
...
print(result)
# Output: "When making financial decisions, I prefer to research extensively before deciding"
```
